...
import { ChartCard, READINESS_SOLID_COLORS } from '@/components/core/readiness-chart-cards';
        ...
        >
            <div className="rounded-[1.75rem] border border-amber-100/70 bg-amber-50/35 p-3 sm:p-4">
                {data.length === 0 || !mounted ? (
                    <div className="flex h-[220px] items-center justify-center text-sm text-amber-900/40">
                    ...
                    <div className="relative">
                        ...
                        <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
                            <span className="text-2xl font-black text-amber-950">{total}</span>
...
export function SensorRadarChart({ profile, hiveName, height = 180 }: SensorRadarChartProps) {
    const [mounted, setMounted] = useState(false);
        ...
        <div className="rounded-[1.75rem] border border-amber-100/70 bg-amber-50/35 p-3 sm:p-4">
            ...
            <div className="mt-2 grid grid-cols-2 gap-x-3 gap-y-0.5">
                {RADAR_AXES.map((axis) => {
                    const raw = RAW_LABELS[axis];
                    return (
                        <span key={axis} className="text-[10px] text-amber-900/60">
                            {axis} {raw.value(profile).toFixed(1)}{raw.unit}
                        </span>
                    );
                })}
            </div>
