...
function validateBeekeeperForm(data: { name: string; email: string; phone: string }) {
 ...
 errors.email = 'Enter a valid email address.'; 
    ...
    if (data.phone && !/^(\+?60|0)[0-9\s-]{8,14}$/.test(data.phone)) {
...
setHarvestLoading(false);
 ...
 setActiveModal({ type: 'view', index }); fetchHarvestRows(beekeepers.data[index].id); 
};
    const close = () => {
 cancelHarvestRef.current(); setActiveModal(null); setHarvestRows(null); 
};
...
return;
 ...
 e.preventDefault(); openView(viewIndex - 1); 
}
 ...
 e.preventDefault(); openView(viewIndex + 1); 
    ...
    const openEdit = (user: User, instant = false) => {
 setEditInstant(instant); editForm.setData({ name: user.name, email: user.email, phone: user.phone ?? '' }); setActiveModal({ type: 'edit', user }); 
};
 ...
 return; 
        ...
        createForm.post(route('admin.beekeepers.store'), { onSuccess: () => {
...
return;
 ...
 editForm.patch(route('admin.beekeepers.update', { user: activeModal.user.id }), { onSuccess: close }); 
};
    const confirmToggle = () => {
...
return;
 ...
 router.patch(route('admin.beekeepers.toggle-status', { user: activeModal.user.id }), {}, { onSuccess: close }); 
};
    const confirmResend = () => {
...
return;
 ...
 router.post(route('admin.beekeepers.resend-invite', { user: activeModal.user.id }), {}, { onSuccess: close }); 
};
    const confirmDelete = () => {
...
return;
 ...
 setDeleting(true); router.delete(route('admin.beekeepers.destroy', { user: activeModal.user.id }), { onFinish: () => {
    ...
    return (
        <AdminLayout>
            ...
            {activeModal?.type === 'edit' && (
                <EditBeekeeperModal isOpen instant={editInstant} form={editForm} onSubmit={openEditConfirm} onClose={() => {
