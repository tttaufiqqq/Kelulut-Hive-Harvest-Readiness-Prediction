...
export default function HivesIndex({
    hives,
    beekeepers,
    species_list,
    sites_list,
    ...
    useEffect(() => {
        ...
        const handler = (e: KeyboardEvent) => {
            ...
            if (e.key === 'ArrowRight' || e.key === 'ArrowDown') {
                e.preventDefault();
                setActiveModal((prev) =>
                    prev?.type === 'view' && prev.index < hives.length - 1
                        ? { type: 'view', index: prev.index + 1 }
                        : prev,
                );
            }
    ...
    const beekeeperOptions = beekeepers.map((b) => ({
        value: String(b.id),
        label: b.name,
    }));
    ...
    const createForm = useForm<HiveFormData>({
        name: '',
        beekeeper_id: '',
        species_id: '',
        site_id: '',
    ...
    const editForm = useForm<HiveFormData>({
        name: '',
        beekeeper_id: '',
        species_id: '',
        site_id: '',
    ...
    const confirmEdit = () => {
        ...
        editForm.post(
            route('admin.hives.update', { hive: activeModal.hive.id }),
            {
                forceFormData: true,
                onSuccess: () => close(),
            },
    ...
    const confirmToggle = () => {
        ...
        router.patch(
            route('admin.hives.toggle-status', { hive: activeModal.hive.id }),
            {},
            { onSuccess: () => close() },
        );
    };
    ...
    const confirmDelete = () => {
        ...
        setDeleting(true);
        router.delete(
            route('admin.hives.destroy', { hive: activeModal.hive.id }),
            {
                onFinish: () => {
                    setDeleting(false);
                    close();
                ...
                <Modal isOpen onClose={close} title="Edit Hive" maxWidth="md">
                    <form onSubmit={submitEdit} className="space-y-4">
                        <SelectField
                            ...
                            onChange={(v) =>
                                editForm.setData('beekeeper_id', v)
                            }
                            options={beekeeperOptions}
