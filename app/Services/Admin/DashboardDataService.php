...
class DashboardDataService
{
    ...
    {
        // hri_summary and harvests are aggregated per site separately, then merged —
        // joining both fact tables directly to hives produces their cross product per
        // hive (hundreds of hri_summary rows x dozens of harvests), which made this
        // query take 50+ seconds once enough history had accumulated.
        $hriBySite = DB::table('hri_summary')
            ->join('hives', 'hives.id', '=', 'hri_summary.hive_id')
            ->groupBy('hives.site_id')
            ->selectRaw('hives.site_id, AVG(hri_summary.avg_hri_value * 100) as avg_hri_pct')
            ->pluck('avg_hri_pct', 'site_id');

        $harvestBySite = DB::table('harvests')
            ->join('hives', 'hives.id', '=', 'harvests.hive_id')
            ->groupBy('hives.site_id')
            ->selectRaw('hives.site_id, SUM(harvests.weight) as total_weight')
            ->pluck('total_weight', 'site_id');

        return DB::table('master_sites')
            ->leftJoin('hives', 'hives.site_id', '=', 'master_sites.id')
            ->groupBy('master_sites.id', 'master_sites.name')
            ->selectRaw('master_sites.id, master_sites.name as site_name, COUNT(DISTINCT hives.id) as hive_count')
            ->get()
            ->map(fn ($row) => [
                'site_name' => $row->site_name,
                'avg_hri_pct' => (int) round((float) ($hriBySite[$row->id] ?? 0)),
                'total_weight' => round((float) ($harvestBySite[$row->id] ?? 0), 1),
