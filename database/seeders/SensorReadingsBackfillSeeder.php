...
/**
 * One-off gap-fill: readings from the last real sensor_logs row up to now,
 * at the same Mon/Wed/Fri cadence the bulk historical data uses (not a
 * denser cadence — a sudden jump in reading frequency would show up as an
 * obvious discontinuity in any daily/monthly reading-count chart).
 *
 * HRI is a bounded random walk per hive (not the flat >80-days-post-harvest
 * "ready" value ProductionDataSeeder's formula would otherwise produce),
 * continuing from each hive's last known prediction so it doesn't jump.
 */
class SensorReadingsBackfillSeeder extends Seeder
{
    private const RANGE_START = '2026-05-22 00:00:00';

    public function run(): void
    {
        $rangeStart = Carbon::parse(self::RANGE_START);
        $now = Carbon::now();
        ...
        foreach ($hives as $hive) {
            ...
            // Scoped to the first day of the range, not the whole range: ProductionDataSeeder's
            // seedTodayReadings() inserts one stray "today" row per hive on every run, which
            // would otherwise satisfy a "any row >= rangeStart" check and skip real backfills.
            $alreadySeeded = DB::table('sensor_logs')
                ->where('hive_id', $hive->id)
                ->whereBetween('record_timestamp', [$rangeStart, $rangeStart->copy()->addDay()])
                ->exists();

            if ($alreadySeeded) {
                $this->command->warn("{$hive->name}: readings from ".self::RANGE_START.' already exist, skipping.');
                continue;
            }
            ...
            $timestamps = $this->buildTimestamps($rangeStart, $now);

            $predRows = [];

            // insertGetId per row (not bulk insert + requery) so each prediction is tied to
            // its own log unambiguously — a requery by timestamp range would also catch
            // ProductionDataSeeder's stray "today" row and misalign the two lists.
            ...
            $this->command->info("{$hive->name}: ".count($predRows).' logs + predictions seeded ('.self::RANGE_START.' to now).');
        }
    }

    /**
     * Mon/Wed/Fri, one reading per day at a business-hour time — matches the
     * bulk historical seeder's cadence so reading density stays continuous
     * across the real-data/backfill boundary.
     *
     * @return Carbon[]
     */
    private function buildTimestamps(Carbon $rangeStart, Carbon $now): array
    {
        $timestamps = [];
    ...
    /**
     * Continue from the hive's last known HRI so the backfill doesn't jump discontinuously.
     */
    ...
    /**
     * Bounded random walk — small drift per reading, occasional larger swing,
     * so readiness moves gradually instead of the flat "ready" the harvest-cycle
     * formula would otherwise lock in for dates this far past the last harvest.
     */
